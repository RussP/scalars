...
object Complex:
  ...
  def cubicRoots(a: Scalar, b: Scalar, c: Scalar, d: Scalar): Vector[Complex] =
    ...
    val f1 = d1 + z1
    val h1 = if (f1 != Complex(0)) f1 else d1 - z1
  ...
  trait implicit1 { implicit def RealToComplex(r: Real): Complex = Complex(r) }
  trait implicit2 { implicit def IntToComplex(i: Int): Complex = Complex(i) }

  object implicits extends implicit1, implicit2
