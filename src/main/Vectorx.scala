...
def Vecx(x: Scalar*) = Vector1(Vector(x: _*))
def Vectorx(x: Scalar*) = Vector1(Vector(x: _*))

implicit class Vector1(self: Vector[Scalar]):
  // adds basic mathematical operations to the Vector[Scalar] class

  import self._

  protected def check(v: Vector[Scalar]) = // check for length compatibility
  ...
  def toUnitVector = if isZero then self else self / mag
