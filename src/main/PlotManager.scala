...
case class PlotManager(name: Text, dir: Text=".", startClean: Bool=true):
  ...
  if startClean then cleanup() // clean out old or unneeded files
  ...
  private def cleanup(numFiles: Int=2) =
    ...
    if numFiles > 1 then deleteFiles(".dat")
    ()
  ...
  private def deleteFiles(ext: Text) =
    ...
    for file <- testDir.listFiles(filter) do file.delete
  ...
  def combinePlots(title: Text="", author: Text="", intro: Text="",
    ...
    val fileNames =
      ...
      for f <- fileNames0 do Process(s"sed -i '1i@version 50125' $f").!

      val fileNames1 = fileNames0.map(x=>replaceAtEnd(x,".dat",".pdf")).sorted

      if fileNames1.isEmpty then { cleanup(); return }

      val sameName = s"$name.pdf"
      val indx = fileNames1.indexOf(sameName)

      if indx >= 0 then // avoid name clash with output file name
    ...
    for fileName <- fileNames if fileName.count(_ == '.') > 1 do
    ...
    for fileName <- fileNames.par do // ".par" for parallel processing
    ...
    val out1 = System.out
    if not(save) then System.setOut(new PrintStream(nullFile))

    Process(s"pdflatex $name").!

    if display then displayPlots

    if save then deleteFiles(".ps") else // save all temp files except .ps
  ...
  def displayPlots =
    ...
    if runningLocally then Runtime.getRuntime.exec(command)
  ...
  private def printLatexHeader(title: Text, author: Text, intro: Text,
    ...
    if title.nonEmpty then out.println(s"\n\\title{\\textbf{$title}}")
    if author.nonEmpty then out.println(s"\\author{$author}")
    if not(showDate) then out.println("\\date{}")

    out.println("\n\\begin{document}\n")

    if title.nonEmpty then out.println("\\maketitle\n")
  ...
  private def replaceAtEnd(txt: Text, ext1: Text, ext2: Text): Text =
    ...
    if not(txt.endsWith(ext1)) then txt else
