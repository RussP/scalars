...
case class ConfigReader(fileName: Text) {
  ...
  override def toString: Text = {
    ...
    if (configFile.exists) for (key <- map.keys.toList.sorted)
      txt ++= ("  " + key + " = " + map(key) + "\n")
    txt
    }
  ...
  def checkConfigPars(abort: Bool, ignore: Text*) {
    ...
    for (opt <- map.keys) if (not(parsRead(opt)) && not(ignored(opt))) {
